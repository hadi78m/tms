...
class TransactionRollbackTest extends TestCase
{
    ...
    public function test_assignment_is_rolled_back_if_audit_fails()
    {
        ...
        $assignee = User::factory()->create(['contractor_id' => 10]);
        ...
        $auditServiceMock->expects($this->any())
            ->method('log')
            ->willThrowException(new \Exception('Audit failed simulate!'));

        $this->app->instance(AuditServiceInterface::class, $auditServiceMock);

        $assignmentService = app(TaskAssignmentService::class);
        $data = new AssignTaskData($task->id, $assignee->id, $actor->id, 'Transaction test');

        try {
            $assignmentService->assign($data, $actor);
        } catch (\Exception $e) {
            $this->assertEquals('Audit failed simulate!', $e->getMessage());
        }

        // Verify the database is completely rolled back
        // 1. Task status should remain Draft
        $this->assertDatabaseHas('tasks', [
            'id' => $task->id,
            'status' => TaskStatus::Draft->value,
        ]);

        // 2. No assignments should be created
        $this->assertEquals(0, TaskAssignment::where('task_id', $task->id)->count());

        // 3. No SLA records should be created
        $this->assertEquals(0, DB::table('sla_records')->where('task_id', $task->id)->count());
    }
}
